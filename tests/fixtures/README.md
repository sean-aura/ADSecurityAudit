...
> **Status on this branch: reference/example data only - not currently
> runnable.** This branch removed offline/`-Snapshot` mode entirely
> (`src/Snapshot.ps1`, `Get-ADSnapshot`, `Invoke-ADRuleSet`,
> `ConvertTo-ADHashtable`, and `Start-ADSecurityAudit`'s `-FromSnapshot`
> parameter are all gone). Everything below this note describes the
> original `-FromSnapshot`-based workflow these fixtures were built for,
> which **no longer works as written** - `tools/Test-ForcedFailFixture.ps1`
> and `tests/ForcedFailFixture.Tests.ps1` were deliberately NOT carried
> over from the `dev` branch for that reason, since both call
> `Start-ADSecurityAudit -FromSnapshot`, a parameter that doesn't exist
> here.
>
> These three JSON files (and `tools/build-forcedfail-fixtures.py`, the
> plain-Python generator that produces them - no PowerShell/module
> dependency) are kept purely as **example data**: a well-thought-out,
> synthetic "31 dirty-area toggles" domain state, useful as a reference
> if/when a live-mode equivalent harness (mocking live `Get-AD*`/`Get-GPO*`
> cmdlets instead of feeding a Snapshot hashtable through
> `Invoke-ADRuleSet`) gets built. Until then, nothing in this repo reads
> or executes these files.
