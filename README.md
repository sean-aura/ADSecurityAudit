...
Thanks to [denandz](https://github.com/denandz) for the patch that independently identified and fixed the `-Server` global-default reliability issue described in the Changelog - hardcoding explicit `-Server` across the `Get-AD*`/`Get-GP*` call sites it touched, which was reconciled into and extended across the rest of the module's live AD/GPO calls.
