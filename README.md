...
- **Schema & persistence-tampering hygiene**: schema classes that let a computer/user object be added as an unrestricted container (`PossSuperiorComputer`/`PossSuperiorUser`, plus the explicit `msExchStorageGroup`/CVE-2021-34470 variant), `defaultSecurityDescriptor` tampering on schema classes, AD Display Specifier (`adminContextMenu`) tampering, and AdminSDHolder ACL inheritance being re-enabled (should never happen — SDProp expects it disabled).
