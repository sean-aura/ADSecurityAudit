...
- **Finished the ForcedFail-fixture removal that v1.25.0's changelog entry claimed but never actually carried out.** When offline/`-Snapshot` mode was removed in v1.25.0, the changelog stated "Removed the ForcedFail fixture ecosystem (`tests/fixtures/ForcedFail-*pct-Snapshot.json`, `tools/build-forcedfail-fixtures.py`, `tools/Test-ForcedFailFixture.ps1`)" - but the JSON fixtures and the Python generator were never actually deleted from the repo, only marked "reference/example data only - not currently runnable" in `tests/fixtures/README.md`. `tools/Test-ForcedFailFixture.ps1` and `tests/ForcedFailFixture.Tests.ps1`, both referenced throughout that README's usage examples, never existed in this repo at all (they were `dev`-branch-only, per the README's own note) - so the README also pointed at commands that would fail if anyone tried to follow them. Confirmed via a full-repo grep that nothing in `src/` or `tests/` actually loads or executes any of these files - the only references left anywhere were historical/attribution comments in `src/DelegationAudits.ps1`, `tests/ConstrainedDelegation.Tests.ps1`, and `tests/DuplicateAceFindingDedup.Tests.ps1` describing how a past bug was originally found, which are left untouched since they're accurate history, not live dependencies.
...
- No functional change: these files were already fully disconnected from the module (confirmed not loaded/executed by anything in `src/` or `tests/` before removal); this just makes the repo's actual contents match what the changelog and module have claimed since v1.25.0.
