...
- **New (`files/15-schema-persistence-tampering.md`):** `'AdminSDHolder Inheritance Re-Enabled'` added directly to `AdminSDAudits.ps1` (reuses the ACL already read for the existing ACE/deny-ACE checks). New `SchemaAudits.ps1` with `'Vulnerable Schema Class Allows Arbitrary Object Creation'` (PossSuperiorComputer/PossSuperiorUser, plus an explicit `msExchStorageGroup`/CVE-2021-34470 check) and `'AD Display Specifier Tampered'` (corrected during self-review to distinguish the two documented `adminContextMenu` registration shapes - a 2-field COM-object registration, which is skipped since it carries no filesystem path at all, versus a 3-field static command registration, which is the shape actually evaluated against the SYSVOL path pattern; the original version would have flagged every default COM-registered entry as a false positive). `'Schema defaultSecurityDescriptor Modified'` is wired up but its reference table (`$Script:SchemaDefaultSecurityDescriptors`) ships **intentionally empty** rather than populated with unverified SDDL defaults from memory - it safely no-ops until populated from a verified schema source; see the doc's own Lab Test Notes.
